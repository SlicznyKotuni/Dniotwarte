...
class QuizApp:
    def __init__(self, root):
        ...
        self.style.configure(
            "TCheckbutton", background="#2E2E2E", foreground="white", font=("Arial", 12)
        )
        self.style.map(
            "TCheckbutton",
        ...
        try:
            with open("pytania.csv", "r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    if len(row) < 11:
                        print(
                            f"Pominięto wiersz z niewystarczającą liczbą kolumn: {row}"
                        )
                        continue
            ...
            print(
                f"Wczytano {len(self.questions)} pytań w {len(self.categories)} kategoriach"
            )
            ...
            for i, category in enumerate(self.categories):
                ...
                if os.path.exists(icon_path):
                    ...
                    print(
                        f"Brak ikony dla kategorii {category}. Oczekiwano pliku: {icon_path}"
                    )
            ...
            messagebox.showerror(
                "Błąd", f"Wystąpił błąd podczas wczytywania pytań: {str(e)}"
            )
    ...
    def setup_layout(self):
        ...
        self.wheel_canvas = tk.Canvas(
            self.wheel_frame, bg="#2E2E2E", highlightthickness=0
        )
        ...
        self.options_canvas = tk.Canvas(
            self.options_container, bg="#2E2E2E", highlightthickness=0
        )

        self.options_scrollbar = ttk.Scrollbar(
            self.options_container, orient="vertical", command=self.options_canvas.yview
        )

        self.options_frame = ttk.Frame(self.options_canvas, style="TFrame")
    ...
    def on_window_resize(self, event):
        ...
        if event.widget == self.root:
            ...
            self.question_label.configure(
                wraplength=self.question_frame.winfo_width() - 40
            )
    ...
    def draw_wheel(self):
        ...
        for i, category in enumerate(self.categories):
            ...
            icon_x = self.wheel_center_x + (self.wheel_radius * 0.7) * math.cos(
                mid_angle_rad
            )
            icon_y = self.wheel_center_y + (self.wheel_radius * 0.7) * math.sin(
                mid_angle_rad
            )
        ...
        pointer = self.wheel_canvas.create_polygon(
            self.wheel_center_x,
            self.wheel_center_y - self.wheel_radius - pointer_size,
            self.wheel_center_x - pointer_size,
            self.wheel_center_y - self.wheel_radius + pointer_size,
            self.wheel_center_x + pointer_size,
            self.wheel_center_y - self.wheel_radius + pointer_size,
            fill="#FF3333",
    ...
    def spin_wheel(self):
        ...
        def animate_spin():
            ...
            if total_angle > 0 and speed > 0.2:
                ...
                category_index = int(
                    ((self.angle + 180) % 360) / (360 / len(self.categories))
                )
        ...
        for category in self.categories:
            ...
            if self.category_icons.get(category):
                icon_label = ttk.Label(
                    category_frame, image=self.category_icons[category]
                )
                icon_label.pack(side=tk.LEFT, padx=5)

            # Nazwa kategorii
            name_label = ttk.Label(category_frame, text=category, style="TLabel")
    ...
    def highlight_category(self, category):
        ...
        selected_frame.name_label.configure(
            foreground="#FF3333"
    ...
    def load_question_from_category(self, category):
        ...
        self.question_label.config(
            text=self.current_question["text"], foreground="white"
        )
    ...
    def check_answer(self):
        ...
        selected_answers = [
            str(i + 1) for i, var in enumerate(self.answer_vars) if var.get()
        ]
        ...
        if correct:
            ...
            correct_options = [
                self.current_question["options"][int(i) - 1]
                for i in self.current_question["correct"]
            ]
            correct_text = ", ".join(correct_options)

            result_text = (
                f"Niepoprawna odpowiedź! Prawidłowe odpowiedzi: {correct_text}"
            )
